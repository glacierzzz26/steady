...
| G10 因子 CRUD/试算/寻优 | 2.3 | ✅ FactorFactory 已接真实 API（mock 移除） | ✅ 2026-08-25：`POST/PUT/DELETE /factors`、`POST /factors/:name/versions`（fork `_v2/_v3` + version 提升 + params 快照）、`POST /factors/:name/switch` 状态机（draft→trial→verified→active→disabled）、`POST /factors/:name/trial|optimize`（DB 队列 `factor_trial`，quant-engine 消费）→ `GET /factor-trials/:id` 轮询 + `GET /factor-trials` 列表；热力图轴=取值最多计算参数。同口径对账：G10 试算 vs G9 factor_stat 三者（pe_ratio/macd_signal/ma_trend）IC/分层/单调一致（见归档 `../stages/2.3b-因子工厂.md`） |
