...
> 关联：[README](README.md) · [进度总表](../../进度总表.md) · [项目详解](../../项目详解.md)
...
**前端接入顺序（2.1）**：Settings → Stocks → StockDetail → Signal → Trade → Dash → Brief → Ops → Backtest；FactorLab/FactorFactory/StrategyFactory 保留 mock，标注「数据待 2.3/2.4」。
...
| G2 /stocks 评分池 | 2.1 | ⬜ |
| G3 详情因子得分 | 2.1 | ⬜ |
| G4 orders/trades 名称 | 2.1 | ⬜ |
| G5 数据健康检查 | 2.1 | ⬜ |
| G6 简报表字段 | 2.1 | ⬜ |
| G7 运维页 | 2.1 | ⬜ |
| G8 回测 T+1 可信度 | 2.2 | ⬜ |
| G9 因子检验分析 | 2.3 | ⬜ |
| G10 因子 CRUD/试算/寻优 | 2.3 | ⬜ |
| G11 策略生命周期+风控 | 2.4 | ⬜ |
