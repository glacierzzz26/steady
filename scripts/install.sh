...
# 自动完成：检测既有部署 → 解压配置包 → 复用/生成 .env → 加载镜像 → compose up -d → 数据库迁移 → 记录版本
...
if docker ps -aq --filter "name=quant-postgres" | grep -q .; then
  ...
  echo "==> 0/6 检测到既有部署：$EXISTING_PROJECT（升级模式，复用数据卷）"
else
  echo "==> 0/6 未检测到既有部署：全新安装"
fi
...
echo "==> 3/6 加载镜像（取最新一份 steady-images*.tar.gz）"
...
echo "==> 4/6 启动服务（run-only compose，VM 不编译）"
if [ "$UPGRADE" = 1 ]; then
...
echo "==> 5/6 数据库迁移（应用 deploy/migrations/*.sql 补齐旧库 schema）"
./scripts/migrate.sh
